...
## Task
Create a script at `scripts/generate-blood-sugar-ndk.ts` that calls the Gamma API to generate a new PDF guide following Kevin's NDK (Nutrient Dense Keto) protocol.

## Gamma API Details
- **API Key:** `sk-gamma-r8dhxiQCmvlhx2ju5ZKNWIGtclWIJnV1WZEqbmskCE`
- **Base URL:** `https://public-api.gamma.app/v1.0`
- **Endpoint:** POST `/generations`
- **Check status:** GET `/generations/{generationId}`

## The Content (Use This Exactly)
...
const response = await fetch("https://public-api.gamma.app/v1.0/generations", {
  ...
  headers: {
    ...
    "X-API-KEY": "sk-gamma-r8dhxiQCmvlhx2ju5ZKNWIGtclWIJnV1WZEqbmskCE",
  },
  body: JSON.stringify({
    inputText: CONTENT_FROM_ABOVE,
    format: "document",
    textMode: "preserve",  // Keep the content as written
    exportAs: "pdf",
    numCards: 12,
...
## Expected Output
When complete, log:
- `gammaUrl` - Link to view/edit in Gamma
- `pdfUrl` - Direct PDF download link

## Run Command
```bash
npx ts-node scripts/generate-blood-sugar-ndk.ts
```

## After Generating
1. Review the PDF at the gammaUrl
2. Update `campaigns/blood-sugar-guide.json` with new URL
3. Update `campaigns/blood-sugar-guide-emails.json` download_link variable
4. Archive or delete the old generic PDF
